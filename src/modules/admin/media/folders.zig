...
test "admin media folders: unauthenticated branches" {
    var create_ctx = Context.init(std.heap.page_allocator, .POST, "/admin/media/folders/create");
    defer create_ctx.deinit();
    try handleCreateFolder(&create_ctx);
    try std.testing.expectEqualStrings("303 See Other", create_ctx.response.status);

    var delete_ctx = Context.init(std.heap.page_allocator, .POST, "/admin/media/folders/delete");
    defer delete_ctx.deinit();
    try handleDeleteFolder(&delete_ctx);
    try std.testing.expectEqualStrings("303 See Other", delete_ctx.response.status);

    var rename_ctx = Context.init(std.heap.page_allocator, .POST, "/admin/media/folders/rename");
    defer rename_ctx.deinit();
    try handleRenameFolder(&rename_ctx);
    try std.testing.expectEqualStrings("303 See Other", rename_ctx.response.status);

    var move_ctx = Context.init(std.heap.page_allocator, .POST, "/admin/media/folders/move");
    defer move_ctx.deinit();
    try handleMoveFolder(&move_ctx);
    try std.testing.expectEqualStrings("303 See Other", move_ctx.response.status);
}

test "admin media folders: public API coverage" {
    _ = handleCreateFolder;
    _ = handleDeleteFolder;
    _ = handleRenameFolder;
    _ = handleMoveFolder;
}
