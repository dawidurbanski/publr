...
test "admin media helpers: buildMediaUrl branches" {
    const base = buildMediaUrl(std.testing.allocator, "grid", null, &.{}, false, null, null, null);
    defer std.testing.allocator.free(base);
    try std.testing.expectEqualStrings("/admin/media", base);

    const full = buildMediaUrl(std.testing.allocator, "list", "f1", &.{ "t1", "t2" }, true, "cats dogs", 2026, 2);
    defer std.testing.allocator.free(full);
    try std.testing.expect(std.mem.indexOf(u8, full, "view=list") != null);
    try std.testing.expect(std.mem.indexOf(u8, full, "folder=f1") != null);
    try std.testing.expect(std.mem.indexOf(u8, full, "tag=t1") != null);
    try std.testing.expect(std.mem.indexOf(u8, full, "unreviewed=1") != null);
    try std.testing.expect(std.mem.indexOf(u8, full, "search=cats+dogs") != null);
}

test "admin media helpers: list and lookup helpers" {
    const terms = [_]media.TermRecord{
        .{ .id = "a", .taxonomy_id = "tax", .slug = "a", .name = "A", .parent_id = null, .description = "", .sort_order = 0 },
        .{ .id = "b", .taxonomy_id = "tax", .slug = "b", .name = "B", .parent_id = "a", .description = "", .sort_order = 0 },
    };

    try std.testing.expect(findFolder(&terms, "a") != null);
    try std.testing.expect(findFolder(&terms, "missing") == null);
    try std.testing.expect(isIdInList("x", &.{ "x", "y" }));
    try std.testing.expect(!isIdInList("z", &.{ "x", "y" }));
    try std.testing.expectEqualStrings("A", findFolderName(&terms, "a"));
    try std.testing.expectEqualStrings("missing", findFolderName(&terms, "missing"));
    try std.testing.expectEqualStrings("A", findTagName(&terms, "a"));
    try std.testing.expectEqualStrings("missing", findTagName(&terms, "missing"));
    try std.testing.expect(isTermSelected("a", &.{ "a", "b" }));
    try std.testing.expect(!isTermSelected("c", &.{ "a", "b" }));

    const removed = removeFromList(std.testing.allocator, &.{ "a", "b", "c" }, "b");
    defer std.testing.allocator.free(removed);
    try std.testing.expectEqual(@as(usize, 2), removed.len);

    const appended = appendToList(std.testing.allocator, &.{ "a", "b" }, "c");
    defer std.testing.allocator.free(appended);
    try std.testing.expectEqual(@as(usize, 3), appended.len);
}

test "admin media helpers: encoding helpers" {
    const decoded = percentDecode(std.testing.allocator, "cats+dogs%2Fbirds");
    defer std.testing.allocator.free(decoded);
    try std.testing.expectEqualStrings("cats dogs/birds", decoded);

    try std.testing.expectEqual(@as(?u4, 10), hexVal('A'));
    try std.testing.expectEqual(@as(?u4, 15), hexVal('f'));
    try std.testing.expectEqual(@as(?u4, null), hexVal('x'));

    var buf: std.ArrayList(u8) = .{};
    defer buf.deinit(std.testing.allocator);
    urlEncode(std.testing.allocator, &buf, "a b/c");
    try std.testing.expectEqualStrings("a+b%2Fc", buf.items);
}

test "admin media helpers: public API coverage" {
    _ = buildMediaUrl;
    _ = buildFolderTree;
    _ = appendFolderChildren;
    _ = buildBreadcrumbs;
    _ = findFolder;
    _ = isIdInList;
    _ = removeFromList;
