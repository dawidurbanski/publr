...
test "admin media bulk: unauthenticated branches" {
    var del_ctx = Context.init(std.heap.page_allocator, .POST, "/admin/media/bulk/delete");
    defer del_ctx.deinit();
    try handleBulkDelete(&del_ctx);
    try std.testing.expectEqualStrings("303 See Other", del_ctx.response.status);

    var add_ctx = Context.init(std.heap.page_allocator, .POST, "/admin/media/bulk/add-tag");
    defer add_ctx.deinit();
    try handleBulkAddTag(&add_ctx);
    try std.testing.expectEqualStrings("303 See Other", add_ctx.response.status);

    var remove_ctx = Context.init(std.heap.page_allocator, .POST, "/admin/media/bulk/remove-tag");
    defer remove_ctx.deinit();
    try handleBulkRemoveTag(&remove_ctx);
    try std.testing.expectEqualStrings("303 See Other", remove_ctx.response.status);

    var move_ctx = Context.init(std.heap.page_allocator, .POST, "/admin/media/bulk/move-folder");
    defer move_ctx.deinit();
    try handleBulkMoveFolder(&move_ctx);
    try std.testing.expectEqualStrings("303 See Other", move_ctx.response.status);
}

test "admin media bulk: public API coverage" {
    _ = handleBulkDelete;
    _ = handleBulkAddTag;
    _ = handleBulkRemoveTag;
    _ = handleBulkMoveFolder;
}
