...
test "admin media list: unauthenticated branch" {
    var ctx = Context.init(std.heap.page_allocator, .GET, "/admin/media");
    defer ctx.deinit();

    try handleList(&ctx);
    try std.testing.expect(std.mem.indexOf(u8, ctx.response.body, "Database not initialized") != null);
}

test "admin media list: public API coverage" {
    _ = handleList;
}
