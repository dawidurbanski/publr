...
const QUERY_TAG = Symbol("query-tag");
const QUERY_INSTANCE = Symbol("query-instance");
const QUERY_META = Symbol("query-meta");
const QUERY_BACKLINK = Symbol("query-backlink");
const MUTATION_TAG = Symbol("mutation-tag");

let currentMutationCtx = null;
let isRestoring = false;

function markBacklink(value) {
  if (!value || typeof value !== "object" || value[QUERY_BACKLINK]) {
    return;
  }
...
function captureSnapshot(target, key, oldValue, newValue) {
  if (target[QUERY_BACKLINK] && newValue) {
    markBacklink(newValue);
  }

  if (!currentMutationCtx || isRestoring || !target[QUERY_BACKLINK]) {
    return;
  }
  ...
  if (!touched) {
    currentMutationCtx.touched.set(target, (touched = new Set()));

    if (Array.isArray(target)) {
      touched.add("length");
      currentMutationCtx.snapshots.push({ target, key: "length", oldValue: target.length });
    }
  }

  if (!touched.has(key)) {
    touched.add(key);
    currentMutationCtx.snapshots.push({ target, key, oldValue });
  }
}

proxyGetHooks.push((target, value) => {
  if (target[QUERY_BACKLINK]) {
    markBacklink(value);
  }
});
...
deepMergeHooks.push((targetValue, sourceValue) => {
  if (!targetValue || typeof targetValue !== "object" || !targetValue[QUERY_INSTANCE]) {
    return false;
  }
...
// prettier-ignore
const QUERY_RESERVED = new Set(["data", "error", "errorObj", "status", "loading", "refetch", "reset", "abort"]);
const MUTATION_RESERVED = new Set(["data", "error", "errorObj", "status", "loading", "reset"]);
...
function registerCacheEntry(entry) {
  for (const tag of entry.tags) {
    ...
    if (!set) {
      cacheRegistry.set(tag, (set = new Set()));
    }
...
function normalizeCacheConfig(spec, queryName) {
  if (spec.cache === undefined) {
    return { tagsFn: null, ttl: null };
  }

  const ok = spec.cache && typeof spec.cache === "object" && typeof spec.cache.tags === "function";

  if (!ok) {
    throw new Error(
      `Publr.query "${queryName || "anonymous"}": cache must be an object with a tags() function — e.g. { tags: () => ['foo'], ttl?: 60 }.`,
    );
  }

  const ttl = typeof spec.cache.ttl === "number" ? spec.cache.ttl : null;

  return { tagsFn: spec.cache.tags, ttl };
}
...
function createQueryInstance(spec, storeName, queryName) {
  ...
  if (storeName) {
    tagStore(metaTarget, storeName, queryName);
  }

  if (spec.initialData && typeof spec.initialData === "object") {
    markBacklink(spec.initialData);
  }
  ...
  function execute() {
    ...
    if (primary && localCache.has(primary)) {
      ...
      const expired = ttl != null && Date.now() - entry.ts >= ttl * 1000;
    ...
    function launch() {
      ...
      try {
        ...
        if (myToken !== token || signal.aborted) {
          return;
        }
    ...
    if (spec.debounce) {
      debounceTimer = setTimeout(launch, spec.debounce);
    } else {
      launch();
    }
  }
  ...
  return new Proxy(
    ...
    {
      get(_target, key) {
        if (key === QUERY_INSTANCE) {
          return true;
        }

        if (key === QUERY_META || key === RAW) {
          return meta;
        }
        ...
        if (QUERY_RESERVED.has(key)) {
          return methods[key] ?? meta[key];
        }
...
function instantiateQueries(obj, storeName) {
  for (const key of Object.keys(obj)) {
    ...
    if (value && typeof value === "object" && value[QUERY_TAG]) {
      obj[key] = createQueryInstance(value, storeName, key);
    }
  }
}
...
function rollbackSnapshots(snapshots) {
  ...
  try {
    function apply(snapshot) {
      ...
      if (proxy) {
        proxy[snapshot.key] = snapshot.oldValue;
      } else {
        snapshot.target[snapshot.key] = snapshot.oldValue;
      }
    }
...
function createMutationInstance(spec, storeName, mutationName) {
  ...
  if (storeName) {
    tagStore(metaTarget, storeName, mutationName);
  }
  ...
  async function runOnce(arg) {
    ...
    const ctx = { snapshots: [], touched: new WeakMap() };
    ...
    try {
      ...
      if (!meta.pending && meta.status === "pending") {
        meta.status = "idle";
      }
    }
  }
  ...
  const callable = function publrMutation() {};
...
function instantiateMutations(actions, storeName) {
  for (const key of Object.keys(actions)) {
    ...
    if (value && typeof value === "object" && value[MUTATION_TAG]) {
      actions[key] = createMutationInstance(value, storeName, key);
    }
  }
}
